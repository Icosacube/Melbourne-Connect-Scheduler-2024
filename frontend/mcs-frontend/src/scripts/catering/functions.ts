...
function reformatCateringResponseData(data: any): Catering {
    const catering: Catering
        ...
        ExpenseDate: data.ExpenseDate ? dayjs(data.ExpenseDate) : defaultCatering.ExpenseDate,
...
export async function createCatering(catering: Catering, id: string): Promise<AxiosResponse> {
    try {
        const toSend: any = { ...catering ,
